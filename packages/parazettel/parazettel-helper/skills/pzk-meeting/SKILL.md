...
5. `pzk_suggest_tags(text=<the note's claim>)` per note (or `pzk_get_all_tags` for the full list) before tagging — reuse existing tags per [tagging.md](../../references/tagging.md); mint a new tag only when the concept is genuinely absent.
